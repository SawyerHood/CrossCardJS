(function(exports) {
    ...
    exports.calculateNextMove = function(deck, board, currentCard, canReserve, playerVal) {
        ...
        for(var i = 0; i < NUM_TO_SIM; i++) {
            ...
            var result = exports.playRandomGame(deck, board, currentCard, canReserve, playerVal);
            if(result !== false) {
                winArray[result] += 1;
            }
        }
    ...
    exports.playRandomGame = function(deck, board, currentCard, canReserve, playerVal) {
        do {
            var randomIndex = Math.random() * (board.size * board.size + 1);
            var isReserve = board.size * board.size == randomIndex;
            var rowAndCol = exports.toRowAndCol(randomIndex, board.size);
